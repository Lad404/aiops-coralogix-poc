...
aiops-venv/
├── aiops_webhook.py        # Main AIOps Flask application (Coralogix → AIOps → Teams/Email)
...
Relationship With systemd & Environment Files

These files are outside the project directory but are critical:

/etc/systemd/system/aiops.service   # systemd service definition
/etc/aiops.env                      # environment variables (secrets & config)
